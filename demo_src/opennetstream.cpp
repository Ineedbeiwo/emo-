...
void OpenNetStreamPrivate::init()
{
    ...
    if (m_library->load()) {
        ...
        m_apis.pOpenSDK_startRealPlayExtend  = (EZSDK_StartRealPlayExtend )m_library->resolve("OpenSDK_StartRealPlayExtend");
        ...
        m_apis.pOpenSDK_startPlayBackExtend  = (EZSDK_StartPlayBackExtend )m_library->resolve("OpenSDK_StartPlayBackExtend");
...
int OpenNetStream::startRealPlayExtend(const QString& session, HWND hPlayWnd,const QString& devSerial, const QString& SuperSerial, const int iChannelNo, const QString& SafeKey)
{
    Q_D(OpenNetStream);
    return d->m_apis.pOpenSDK_startRealPlayExtend(session.toStdString().c_str(), hPlayWnd, devSerial.toStdString().c_str(), SuperSerial.toStdString().c_str(),iChannelNo, SafeKey.toStdString().c_str());
}
...
int OpenNetStream::startPlayBackExtend(const QString& session, HWND hPlayWnd, const QString& devSerial, const QString& superSerial,const int iChannelNo, const QString& safekey, const QString& startTime, const QString& stopTime)
{
    Q_D(OpenNetStream);
    return d->m_apis.pOpenSDK_startPlayBackExtend(session.toStdString().c_str(), hPlayWnd,
        devSerial.toStdString().c_str(), superSerial.toStdString().c_str(),iChannelNo, safekey.toStdString().c_str(), 
        startTime.toStdString().c_str(), stopTime.toStdString().c_str());
}
...
int OpenNetStream::setPlayBackScale(const QString& session,int iScale, const char* szOsdTime, int mode)
{
    Q_D(OpenNetStream);
    return d->m_apis.pOpenSDK_SetPlayBackScale(session.toStdString().c_str(),iScale, szOsdTime, mode);
} 
