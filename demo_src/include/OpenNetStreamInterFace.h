...
/** 
 *  \brief 针对摄像头进行预览(扩展接口),不适用于国标设备
           异步接口，返回值只是表示操作成功，不代表播放成功
           如果接口返回-1, 结果根据消息回调函数msgid判断, 若msgid=INS_PLAY_START, 表示成功, 若msgid=INS_PLAY_EXCEPTION, 表示失败
 *  \param[in] szSessionId   会话ID
 *  \param[in] hPlayWnd      播放窗口句柄, 如果窗口句柄为NULL表示纯粹取流，不做播放
 *  \param[in] dev_info      设备信息结构体,包括序列号 通道号及验证码等信息参数
 *  \return 0表示成功，非0表示失败
 *  \sa OpenSDK_stopRealPlay
 *	\sa OpenSDK_SetDataCallBack
 */
...
/** 
 *  \brief 按录像源进行搜索，支持本地录像和云存储录像搜索,搜索本地录像时如果关联NVR优先搜索NVR录像，不适用于国标设备,支持长序列号设备录像搜索
           同步接口,不适用于国标设备
 ...
 *  \param[in] szSessionId 会话Id
 *  \param[in] iScale 回放倍速,1(正常倍速),2(2倍速),3(1/2倍速),4(4倍速),5(1/4倍速),6(8倍速),7(1/8倍速),8(16倍速),9(1/16倍速)
       云存储回放只支持1,4,8,16倍速切换
