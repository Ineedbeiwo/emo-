...
enum MessageType
{
    ...
    INS_VTDU_STREAM_EXCEPTION     =115,  ///< 取流过程中设备与流媒体服务之间的链接异常断开
};
...
/** \struct  _DevInfo 
 *  \brief 取流传入的设备信息参数结构体
 */
typedef struct _ezDevInfo
{
    const char* szSuperSerial; ///<  主设备的序列号,非边缘设备勿传该参数
    const char* szDevSerial;   ///<  设备序列号
    const char* szSafeKey;     ///<  验证码
    int         iChannelNo;     ///<  设备通道号        
} ezDevInfo, *lp_ezDevInfo;
/** \enum  AlarmType 
 *  \brief 开放SDK告警类型定义
 */
enum AlarmType
