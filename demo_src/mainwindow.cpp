...
void MainWindow::messageHandler(const char *szSessionId, unsigned int iMsgType, unsigned int iErrorCode, const char *pMessageInfo, void *pUser)
{
    ...
    case INS_VTDU_STREAM_EXCEPTION:
...
void MainWindow::videoDataHandler(DataType enType, char* const pData, int iLen, void* pUser)
{
    ...
    if(NET_DVR_RECV_202 == enType || NET_DVR_RECV_201 == enType)
    {
        qDebug() << __LINE__ << __FUNCTION__ <<"!!!!!!!!!recv 202 or 201 msg ,type:"<< enType;
    }
...
void MainWindow::on_startPlayBackButton_clicked()
{
    ...
    if(1 == LocalSetting::getInstance()->getHubMode())
    {
        ...
        m_getSingleDevInfo->getDevSerialFromLogin(devSerial);
        m_getSingleDevInfo->getChannelFromLogin(iChannelNo);
        m_getSingleDevInfo->getSuperSerialFromLogin(superSerial);
    }
    ...
    if(1 == LocalSetting::getInstance()->getHubMode())
    {
        iRet = OpenNetStream::getInstance()->startPlayBackExtend(m_sessionId, hWnd, devSerial,superSerial,iChannelNo, safekey, beginTime, endTime);
    }
    else
    {
...
int MainWindow::setPlayBackScale(int iScale)
{
    ...
    if(m_bCloudPlayBack)
    {
        ...
        iFastMode = LocalSetting::getInstance()->getCloudFastPlayMode();
        qDebug() << __LINE__ << __FUNCTION__ << "!!!!!!!!!!!!!set cloud Play Back Speed:"<<iScale<<">>>>>>>>>>>>>>FastMode:"<<iFastMode;
        iRet = OpenNetStream::getInstance()->setPlayBackScale(m_sessionId, iScale, szTime, iFastMode);
    }
    else
    {
        qDebug() << __LINE__ << __FUNCTION__ << "<<<<<<<<<<<<<<<<<<<<<set device record Play Back Speed:"<<iScale;
        iRet = OpenNetStream::getInstance()->setPlayBackScale(m_sessionId, iScale, NULL, 0);
    }
...
int MainWindow::startRealPlay(int videoLevel)
{
    ...
    qDebug()<<">>>>>>>>>>>>>>serial:"<<devSerial<<endl;
    qDebug()<<">>>>>>>>>>>>>>superSerial:"<<superSerial; 
   
    //int iRet = OpenNetStream::getInstance()->startRealPlay(m_sessionId, hWnd, devSerial, iChannelNo, safekey);
    int iRet = 0;
    if(1 == LocalSetting::getInstance()->getHubMode())
    {
        iRet = OpenNetStream::getInstance()->startRealPlayExtend(m_sessionId, hWnd, devSerial, superSerial, iChannelNo, safekey);
    } 
    else
    {
        iRet = OpenNetStream::getInstance()->startRealPlay(m_sessionId, hWnd, devSerial, iChannelNo, safekey);
    }
	if(0 != iRet)
	{
...
void MainWindow::on_getSingleDeviceButton_clicked()
{
    ...
    OpenNetStream::getInstance()->setAppId(AppKey);
